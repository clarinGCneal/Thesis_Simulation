...
int main() {
    ...
    for (int num_particles : nList) {
        for (int num_steps : stepList) {
            ...
            // Loop over all particles sequentially
            for (int p = 0; p < num_particles; p++) {
                // Positions for each step
                std::vector<double> X(num_steps, 0.0);
                std::vector<double> Y(num_steps, 0.0);

                // Sequential random‐walk updates (dependencies prevent vectorization)
                ...
                // This loop is independent between steps and can be vectorized.
                #pragma omp simd
                for (int step = 1; step < num_steps; step++) {
                    msd[step] += X[step] * X[step] + Y[step] * Y[step];
                }
            }
